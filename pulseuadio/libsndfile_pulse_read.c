...
int main(int argc, char *argv[]) {
    ...
    playstream = pa_stream_new(pa_ctx,  "Playback", &ss, &l_SChannelMap);
    ...
    r = pa_stream_connect_playback(playstream, m_SSinkList[1].name, &bufattr,
                                   PA_STREAM_INTERPOLATE_TIMING
                                   | PA_STREAM_ADJUST_LATENCY
                                   | PA_STREAM_AUTO_TIMING_UPDATE, NULL, NULL);

    if (r < 0) {
        printf("Can't connect to server. Trying with another parameters\n");
